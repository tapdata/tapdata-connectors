...
public class MysqlWrapperType extends WrapperType {
    public MysqlWrapperType() {
        this.ddlTypes.add(new DDLType(DDLType.Type.ADD_COLUMN, "alter\\s+table\\s+.*\\s+add\\s+(column){0,1}\\s+((?!(index|constraint|fulltext|key|spatial|partition)).)+.+", false, "ADD [COLUMN] (col_name column_definition,...)", MysqlAddColumnDDLWrapper.class));
        ...
        this.ddlTypes.add(new DDLType(DDLType.Type.DROP_COLUMN, "alter\\s+table\\s+.*\\s+drop\\s+(column){0,1}\\s+((?!(check|constraint|index|key|primary\\skey|foreign\\skey)).)+.+", false, "DROP [COLUMN] col_name", MysqlDropColumnDDLWrapper.class));
    }
}
