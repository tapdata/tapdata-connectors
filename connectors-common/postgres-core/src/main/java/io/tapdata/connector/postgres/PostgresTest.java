...
public class PostgresTest extends CommonDbTest {
    ...
    public Boolean testReadPrivilege() {
        try {
            AtomicBoolean isDBA = new AtomicBoolean();
            jdbcContext.queryWithNext(PG_SUPER_USER, resultSet -> isDBA.set(resultSet.getBoolean(1)));
            if (isDBA.get()) {
                consumer.accept(testItem(TestItem.ITEM_READ, TestItem.RESULT_SUCCESSFULLY, "Current user has DBA privilege"));
                return true;
            }
    ...
    private final static String PG_SUPER_USER = "select rolsuper from pg_roles where rolname=current_user";
