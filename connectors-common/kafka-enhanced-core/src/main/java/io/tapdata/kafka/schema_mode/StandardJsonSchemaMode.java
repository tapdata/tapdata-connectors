 ...
 */
public class StandardJsonSchemaMode extends AbsSchemaMode {

    private StandardSerializer serializer = new StandardSerializer();
    private StandardDeserializer deserializer = new StandardDeserializer();

    public StandardJsonSchemaMode(IKafkaService kafkaService) {
        super(KafkaSchemaMode.STANDARD_JSON, kafkaService);
    }
        ...
        kafkaService.<Long, Object>sampleValue(Collections.singletonList(table), null, record -> {
            if (null != record) {
                if (record.value() instanceof String) {
                    Map<String, Object> data = TapSimplify.fromJson((String) record.value(), Map.class);
    ...
    public TapEvent toTapEvent(ConsumerRecord<?, ?> consumerRecord) {
        ...
        if (value instanceof String) {
            return deserializer.deserialize(consumerRecord.topic(), ((String) value).getBytes());
        } else if (null == value) {
            return null;
        }
    ...
    public List<ProducerRecord<Object, Object>> fromTapEvent(TapTable table, TapEvent tapEvent) {
        ...
        return List.of(new ProducerRecord<>(topic, computePartition(createKafkaKey(data, table), kafkaService.getConfig().getNodePartitionSize()), ts, createKafkaKeyValueMap(data, table), new String(serializer.serialize(topic, tapEvent)), headers));
    }

    public ProducerRecord<Object, Object> fromTapDDLEvent(TapDDLEvent ddlEvent) {
        return new ProducerRecord<>(ddlEvent.getTableId(), null, ddlEvent.getTime(), null, serializer.serialize(ddlEvent.getTableId(), ddlEvent));
    }
