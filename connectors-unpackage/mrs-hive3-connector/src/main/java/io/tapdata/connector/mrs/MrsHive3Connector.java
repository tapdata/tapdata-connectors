...
public class MrsHive3Connector extends HiveConnector {
    private static final String TAG = MrsHive3Connector.class.getSimpleName();
    private MrsHive3Write mrsHive3Write;
    ...
    public void onStart(TapConnectionContext connectionContext) {
        ...
        mrsHive3Config = new MrsHive3Config(firstConnectorId).load(connectionContext.getConnectionConfig());
        hiveJdbcContext = new MrsHive3JdbcContext(mrsHive3Config);
        commonDbConfig = hiveConfig;
        jdbcContext = hiveJdbcContext;
        commonSqlMaker = new CommonSqlMaker('`');
        mrsHive3Write = new MrsHive3Write(hiveJdbcContext, mrsHive3Config);
    ...
    public void onStop(TapConnectionContext connectionContext) {
        ...
        if(mrsHive3Write !=null){
            mrsHive3Write.onDestroy();
        }
    }
    @Override
    ...
    public ConnectionOptions connectionTest(TapConnectionContext connectionContext, Consumer<TestItem> consumer) {
        ...
        MrsHive3Config hiveConfig = new MrsHive3Config(null).load(connectionContext.getConnectionConfig());
        try (
                MrsHive3Test hive3Test = new MrsHive3Test(hiveConfig, consumer)
        ) {
    ...
    public CreateTableOptions createTableV2(TapConnectorContext tapConnectorContext, TapCreateTableEvent tapCreateTableEvent) {
        ...
        try {
            if (jdbcContext.queryAllTables(Arrays.asList(tapTable.getId())).size() > 0) {
                ...
                String sql = "CREATE TABLE IF NOT EXISTS " + mrsHive3Config.getDatabase() + "." + tapTable.getId() + "(" + commonSqlMaker.buildColumnDefinition(tapTable, true);
    ...
    public void dropTable(TapConnectorContext tapConnectorContext, TapDropTableEvent tapDropTableEvent) {
        try {
            jdbcContext.execute("DROP TABLE IF EXISTS " + mrsHive3Config.getDatabase() + "." + tapDropTableEvent.getTableId());
    ...
    public void clearTable(TapConnectorContext tapConnectorContext, TapClearTableEvent tapClearTableEvent) {
        try {
            if (hiveJdbcContext.queryAllTables(Collections.singletonList(tapClearTableEvent.getTableId())).size() == 1) {
                hiveJdbcContext.execute("TRUNCATE TABLE " + mrsHive3Config.getDatabase() + "." + tapClearTableEvent.getTableId() + "\"");
            }
        } catch (Throwable e) {
            e.printStackTrace();
