...
function insertRecord(connectionConfig, nodeConfig, eventDataMaps, settings){
    let tableSettings = {};
    if(settings){
    ...
    if (eventDataMaps) {
        eventDataMaps.forEach((eventData) => {
            if (eventData.afterData && eventData.afterData[tableSettings.keys[0]]) {
                datas.push(eventData.afterData);
            }
        });
        doPost = datas.length > 0;
    }
    if(tableSettings && tableSettings.keys && tableSettings.keys.length > 0 && doPost){
