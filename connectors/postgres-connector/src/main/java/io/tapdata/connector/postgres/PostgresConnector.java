...
public class PostgresConnector extends CommonDbConnector {
    ...
    protected void afterInitialSync(TapConnectorContext connectorContext, TapTable tapTable) throws Throwable {
        ...
        autoIncFields.forEach(field -> {
            try {
                jdbcContext.queryWithNext("select max(" + field + ") from " + getSchemaAndTable(tapTable.getId()), resultSet -> {
                    jdbcContext.execute("ALTER TABLE " + getSchemaAndTable(tapTable.getId()) + " ALTER COLUMN \"" + field + "\" SET GENERATED BY DEFAULT RESTART WITH " + (resultSet.getLong(1) + postgresConfig.getAutoIncJumpValue()));
                });
            } catch (SQLException e) {
