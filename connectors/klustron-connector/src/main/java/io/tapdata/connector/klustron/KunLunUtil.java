 ...
 * @version v1.0 2026/3/4 12:09 Create
 * @description
 */
public class KunLunUtil extends JdbcUtil {
    ...
    public static void testHostPortForMasterSlave(MysqlConfig mysqlConfig) {
        ArrayList<LinkedHashMap<String, Object>> masterSlaveAddress = mysqlConfig.getMasterSlaveAddress();
        ArrayList<LinkedHashMap<String, Object>> availableMasterSlaveAddress = Optional.ofNullable(mysqlConfig.getAvailableMasterSlaveAddress()).orElse(new ArrayList<>());
        for (LinkedHashMap<String, Object> hostPort : masterSlaveAddress) {
            try {
                if (EmptyKit.isEmpty(hostPort)) {
                    continue;
                }
                NetUtil.validateHostPortWithSocket(String.valueOf(hostPort.get("host")), (Integer) (hostPort.get("port")));
    ...
    public static void buildMasterNode(MysqlConfig mysqlConfig, java.util.HashMap<String, KunLunMysqlContext> contextMapForMasterSlave) {
        ...
        if (DeployModeEnum.fromString(deploymentMode) == DeployModeEnum.MASTER_SLAVE) {
            ...
            ArrayList<LinkedHashMap<String, Object>> availableMasterSlaveAddress = mysqlConfig.getAvailableMasterSlaveAddress();
            ...
            HashSet<LinkedHashMap<String, Object>> masterNode = new HashSet<>(availableMasterSlaveAddress);
            Map<String, Object> masterHostPortAndStatus;
            int count = 1;
            boolean needQuerySlaveStatus = availableMasterSlaveAddress.size() == 1;
            while ((count < 3 && masterNode.size() != 1) || needQuerySlaveStatus) {
                Iterator<LinkedHashMap<String, Object>> iterator = masterNode.iterator();
                while (iterator.hasNext()) {
                    LinkedHashMap<String, Object> address = iterator.next();
                    ...
                    try {
                        ...
                        if (mysqlJdbcContext.queryIsPrimary()) {
                            primary.put("host", host);
                            primary.put("port", port);
                        }
                    } catch (Throwable e) {
            ...
            } else if (masterNode.size() > 1 && !primary.isEmpty()) {
                mysqlConfig.setHost(String.valueOf(primary.get("host")));
                mysqlConfig.setPort((Integer) primary.get("port"));
                mysqlConfig.setMasterNode(primary);
            } else {
                LinkedHashMap<String, Object> master = masterNode.stream().findFirst().get();
                mysqlConfig.setHost(String.valueOf(master.get("host")));
                mysqlConfig.setPort((Integer) master.get("port"));
                mysqlConfig.setMasterNode(master);
            }
        }
    }

    public static ArrayList<Map<String, Object>> compareMasterSlaveCurrentTime(MysqlConfig mysqlConfig, java.util.HashMap<String, KunLunMysqlContext> contextMapForMasterSlave) {
        ...
        if (DeployModeEnum.fromString(deploymentMode) == DeployModeEnum.MASTER_SLAVE) {
            ...
            for (LinkedHashMap<String, Object> address : availableMasterSlaveAddress) {
                String host = String.valueOf(address.get("host"));
                int port = (Integer) address.get("port");
                mysqlJdbcContext = contextMapForMasterSlave.get(host + ":" + port);
                try {
    ...
    public static java.util.HashMap<String, KunLunMysqlContext> buildContextMapForMasterSlave(MysqlConfig mysqlConfig) {
        if (null == mysqlConfig) return null;
        java.util.HashMap<String, KunLunMysqlContext> contextMap = new java.util.HashMap<>();
        String deploymentMode = mysqlConfig.getDeploymentMode();
        if (DeployModeEnum.fromString(deploymentMode) == DeployModeEnum.MASTER_SLAVE) {
            testHostPortForMasterSlave(mysqlConfig);
            ArrayList<LinkedHashMap<String, Object>> availableMasterSlaveAddress = mysqlConfig.getAvailableMasterSlaveAddress();
            ...
            for (LinkedHashMap<String, Object> address : availableMasterSlaveAddress) {
                ...
                mysqlConfig.setHost(host);
                mysqlConfig.setPort(port);
                contextMap.putIfAbsent(host + ":" + port, new KunLunMysqlContext(mysqlConfig));
            }
        }
        return contextMap;
    }
