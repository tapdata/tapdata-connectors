...
import io.tapdata.pdk.apis.exception.testItem.TapTestCDCPrivilegeEx;
import io.tapdata.pdk.apis.exception.testItem.TapTestCreateTablePrivilegeEx;
 ...
 * @version v1.0 2026/3/4 16:36 Create
 * @description
 */
public class KunLunConnectionTest extends CommonDbTest {
    protected static final String TAG = KunLunConnectionTest.class.getSimpleName();
    ...
    public KunLunConnectionTest(MysqlConfig mysqlConfig, Consumer<TestItem> consumer, ConnectionOptions connectionOptions) {
        super(mysqlConfig, consumer);
        jdbcContext = new KunLunMysqlContext(mysqlConfig);
    ...
    public Boolean testReadPrivilege() {
        return true;//WriteOrReadPrivilege("read");
    }
    ...
    public Boolean testCDCPrivileges() {
        ...
        try {
            ...
            List<KunLunConnectionTest.CdcPrivilege> cdcPrivileges = new ArrayList<>(Arrays.asList(KunLunConnectionTest.CdcPrivilege.values()));
            jdbcContext.normalQuery(getGrantsSql(), resultSet -> {
                while (resultSet.next()) {
                    String grantSql = resultSet.getString(1);
                    Iterator<KunLunConnectionTest.CdcPrivilege> iterator = cdcPrivileges.iterator();
                    while (iterator.hasNext()) {
                        boolean match = false;
                        KunLunConnectionTest.CdcPrivilege cdcPrivilege = iterator.next();
            ...
            if (null == testItem.get()) {
                if (CollectionUtils.isNotEmpty(cdcPrivileges) && cdcPrivileges.size() > 1) {
                    for (KunLunConnectionTest.CdcPrivilege cdcPrivilege : cdcPrivileges) {
