...
public class TidbWrapperType extends WrapperType {
    public TidbWrapperType() {
        this.ddlTypes.add(new DDLType(DDLType.Type.CHANGE_COLUMN, "alter\\s+table\\s+.*\\s+change(\\s+column){0,1}.+", false, "CHANGE [COLUMN] old_col_name new_col_name column_definition", TidbAlterColumnNameDDLWrapper.class, TidbAlterColumnAttrsDDLWrapper.class));
        ...
        this.ddlTypes.add(new DDLType(DDLType.Type.ALTER_COLUMN, "alter\\s+table\\s+.*\\s+modify(\\s+column){0,1}.+", false, "MODIFY [COLUMN] col_name column_definition", TidbAlterColumnAttrsDDLWrapper.class));
