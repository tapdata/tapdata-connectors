package io.tapdata.connector.mrskafka.core;
...
import io.tapdata.connector.mrskafka.config.*;
...
import io.tapdata.connector.mrskafka.util.ObjectUtils;
...
public class MrsKafkaService extends AbstractMqService {
    ...
    public TestItem testConnect() {
        if (((MrsKafkaConfig) mqConfig).getHuaweiKerberos()) {
            try {
                MrsKerberosUtil.checkKDCDomainsBase64(((MrsKafkaConfig) mqConfig).getHuaweiKrb5Conf());
        ...
        MrsAdminConfiguration configuration = new MrsAdminConfiguration(((MrsKafkaConfig) mqConfig), connectorId);
    ...
    public void init() throws Throwable {
        try {
            MrsProducerConfiguration mrsProducerConfiguration = new MrsProducerConfiguration(mrsKafkaConfig, connectorId);
            Map<String, Object> pros = mrsProducerConfiguration.build();
            this.kafkaProducer = new KafkaProducer<>(pros);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void loadTables(int tableSize, Consumer<List<TapTable>> consumer) throws Throwable {
        ...
        MrsSchemaConfiguration schemaConfiguration = new MrsSchemaConfiguration(((MrsKafkaConfig) mqConfig), connectorId);
        submitPageTables(tableSize, consumer, schemaConfiguration, destinationSet);
    }

    protected void submitPageTables(int tableSize, Consumer<List<TapTable>> consumer, MrsSchemaConfiguration schemaConfiguration, Set<String> destinationSet) {
                ...
                Callback callback = (metadata, exception) -> {
                    try {
                        if (EmptyKit.isNotNull(exception)) {
                            mrsKafkaExceptionController.collectTerminateByServer(exception);
                            mrsKafkaExceptionController.collectUserPwdInvalid(mqConfig.getMqUsername(), exception);
                            mrsKafkaExceptionController.collectWritePrivileges("writeRecord", Collections.emptyList(), exception);
                            mrsKafkaExceptionController.collectWriteLength(null, null, event, exception);
...
//            this.produce(null,tapRecordEvents,tapTable,writeListResultConsumer,isAlive);
    ...
    public void produce(TapConnectorContext connectorContext, List<TapRecordEvent> tapRecordEvents, TapTable tapTable, Consumer<WriteListResult<TapRecordEvent>> writeListResultConsumer, Supplier<Boolean> isAlive) {
        ...
        String script = ((MrsKafkaConfig) mqConfig).getScript();
    ...
    public void consumeOne(TapTable tapTable, int eventBatchSize, BiConsumer<List<TapEvent>, Object> eventsOffsetConsumer) throws Throwable {
        ...
        MrsConsumerConfiguration mrsConsumerConfiguration = new MrsConsumerConfiguration(((MrsKafkaConfig) mqConfig), connectorId, true);
        KafkaConsumer<byte[], byte[]> kafkaConsumer = new KafkaConsumer<>(mrsConsumerConfiguration.build());
    ...
    public void streamConsume(List<String> tableList, int eventBatchSize, BiConsumer<List<TapEvent>, Object> eventsOffsetConsumer) {
        consuming.set(true);
        MrsConsumerConfiguration consumerConfiguration = new MrsConsumerConfiguration(((MrsKafkaConfig) mqConfig), connectorId, true);
