...
public class Krb5Util {
    ...
    /**
     * 刷新 Kerberos 配置（使用反射访问 sun.security.krb5.Config）
     * 兼容 JDK 9+ 模块化系统
     */
    private static void refreshKrb5Config() {
        try {
            // 使用反射访问 sun.security.krb5.Config.refresh()
            // 这是为了兼容 JDK 9+ 的模块化系统
            ...
            TapLogger.debug(TAG, "Kerberos config refreshed successfully");
        } catch (ClassNotFoundException e) {
            // JDK 中没有 sun.security.krb5.Config 类，可能是非标准 JDK
            TapLogger.warn(TAG, "sun.security.krb5.Config class not found, skip refresh");
        } catch (NoSuchMethodException e) {
            TapLogger.warn(TAG, "sun.security.krb5.Config.refresh() method not found, skip refresh");
        } catch (Exception e) {
            // 反射调用失败，记录警告但不抛出异常
            TapLogger.warn(TAG, "Failed to refresh Kerberos config via reflection: " + e.getMessage());
        }
    }
     ...
     * @param conf        kafka配置
     */
    public static void updateKafkaConf(String serviceName, String principal, String krb5Path, String krb5Conf, Properties conf) {
        ...
        if (null != currentRealms && currentRealms.containsKey("kdc")) {
            ...
            // 使用反射刷新配置，避免直接依赖 sun.security.krb5.Config
            refreshKrb5Config();
        } else {
