...
public class PaimonService implements AutoCloseable {
     ...
     * JVM 中多个 writer 同时写同一物理表。任务可以迁移到另一个 Engine，但此静态表不跨进程共享；
     * 新 Engine 开始写入前，宿主必须证明旧实例已停止提交，或提供真正的分布式 fencing。
     * Spill 目录锁只保护本地删除，不能作为远端表写入的租约。
     */
    ...
    /** 关闭等待的 INFO 观察间隔，不是停止的总时限。 */
    private static final long STOP_PROGRESS_INTERVAL_SECONDS = 5L;
    /** close worker 脱离短生命周期 PDK 任务 ThreadGroup，保持 UGI/TCCL 的创建上下文。 */
    ...
    /** 资源清理证明尚未成立的物理表；不启动后台回收者或定时放行。 */
    private final Set<String> unsafeResourceOwners = ConcurrentHashMap.newKeySet();
    ...
    private volatile PaimonStopController closeOperation;
    private final PaimonStopController stopController;
    private final PaimonStopResources stopResources = new PaimonStopResources();
    private PaimonStopResources.Scope catalogScope;
    ...
    PaimonService(
        ...
        this(config, log, clock, retryWaiter, schedulerExecutorFactory, System::nanoTime);
    }
    ...
    PaimonService(
            ...
            LongSupplier clock,
            RetryWaiter retryWaiter,
            PaimonAsyncCommitScheduler.ExecutorFactory schedulerExecutorFactory, LongSupplier stopClock) {
        this.log = log;
        this.config = config;
        config.validateStopBudgets();
        this.stopController = new PaimonStopController(serviceWriterOwner,
                config.getStopTimeoutSeconds(), config.getFinalCompactionTimeoutSeconds(),
                config.getCompactionCancelGraceSeconds(), stopClock);
        this.stopController.attachResourceRoot(this);
    ...
    public void init() throws Exception {
        try { initializeCatalog(); }
            ...
            if (catalogScope != null && !catalogScope.hasResources()) { catalogScope.completed(); catalogScope = null; }
            cleanupAfterInitializationFailure(failure);
            rethrow(failure);
        }
    }

    private synchronized void initializeCatalog() throws Exception {
        ...
        // Create catalog
            catalogScope = stopResources.scope("catalog", stopController);
            catalog = catalogScope.create("create catalog", () -> CatalogFactory.createCatalog(context));
            stopController.checkAction("publish initialized service");
    ...
    /**
     * Remove stale {@code paimon-io-*} spill directories under every root the runtime can spill
     * into: the global {@code diskTmpDir} plus each per-table override from the table node
     * config, because contexts and the HASH_DYNAMIC preflight resolve their IOManager roots
     * through the per-table value. Best-effort: failures are logged, never thrown.
     */
    private void cleanupStaleSpillDirs() {
        try {
            Set<String> roots = new LinkedHashSet<>();
            collectTmpDirRoots(config.getDiskTmpDir(), roots);
            Map<String, DataMap> tableConfig = config.getTableConfig();
            if (tableConfig != null) {
                for (DataMap table : tableConfig.values()) {
                    if (table != null && table.get("diskTmpDir") instanceof String) {
                        collectTmpDirRoots((String) table.get("diskTmpDir"), roots);
                    }
                }
            }
            int deleted = PaimonSpillDirCleaner.cleanupStaleSpillDirs(
                    roots.toArray(new String[0]),
                    PaimonSpillDirCleaner.DEFAULT_STALE_GRACE_MS,
                    (path, bytes) -> log.info("Removed stale Paimon spill dir {} ({} bytes)", path, bytes));
            if (deleted > 0) {
                log.info("Cleaned up {} stale Paimon spill dir(s) under {}", deleted, roots);
            }
        } catch (Exception e) {
    ...
    /** Splits one configured tmp-dir list (comma / path-separator) into canonical scan roots. */
    private void collectTmpDirRoots(String configuredTmpDirs, Set<String> roots) {
        for (String root : PaimonSpillDirCleaner.splitTmpDirRoots(
                PaimonSpillDirCleaner.resolveTmpDirs(configuredTmpDirs))) {
            java.io.File directory = new java.io.File(root);
            // canonicalize 不能把 cleaner 拒绝的符号链接根转换成可清理的真实目录。
            if (java.nio.file.Files.isSymbolicLink(directory.toPath())) { continue; }
            try { roots.add(directory.getCanonicalPath()); }
            catch (java.io.IOException failure) {
                log.warn("Cannot canonicalize Paimon spill root {}: {}", root, failure.getMessage());
            }
        }
    }
...
//				options.set("fs.s3a.retry.interval", "1000");
     ...
     */
    private Configuration buildHadoopConfiguration() throws IOException {
        ...
        // 用户属性可覆盖缓存/认证选项；仅替换标准实现的初始化入口，不覆盖自定义 FS。
        // Paimon HadoopFileIO 按 authority 懒创建 FS，单独移动 Catalog 构造线程不足以覆盖该路径。
        // Source: https://github.com/apache/paimon/blob/release-1.3.2/paimon-common/src/main/java/org/apache/paimon/fs/hadoop/HadoopFileIO.java
        if (org.apache.hadoop.fs.FileSystem.getFileSystemClass("s3a", conf)
                == org.apache.hadoop.fs.s3a.S3AFileSystem.class) {
            conf.setClass("fs.s3a.impl",
                    io.tapdata.connector.paimon.fs.PaimonS3AFileSystem.class,
    ...
    public int getTableCount() throws Exception {
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("getTableCount")) {
            return getTableCountAdmitted();
        }
    }

    private int getTableCountAdmitted() throws Exception {
        String database = config.getDatabase();

        // Check if database exists
        try {
            stopController.call("catalog getDatabase", () -> catalog.getDatabase(database));
        } catch (Catalog.DatabaseNotExistException e) {
        ...
        List<String> tables = stopController.call("catalog listTables", () -> catalog.listTables(database));
    ...
    public List<TapTable> discoverTables(List<String> tableNames) throws Exception {
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("discoverTables")) {
            return discoverTablesAdmitted(tableNames);
        }
    }

    private List<TapTable> discoverTablesAdmitted(List<String> tableNames) throws Exception {
        ...
        // Get all tables in database
        List<String> allTables = stopController.call("catalog listTables", () -> catalog.listTables(database));
        ...
        for (String tableName : allTables) {
            try {
                ...
                tables.add(tapTable);
            } catch (PaimonStopController.FrozenException frozen) { throw frozen;
            } catch (Exception e) {
    ...
    public boolean createTable(TapTable tapTable) throws Exception {
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("createTable")) {
    ...
    private boolean createTableInternal(TapTable tapTable) throws Exception {
        ...
        try {
            ...
            // Database does not exist, create it
            stopController.run("catalog createDatabase", () -> catalog.createDatabase(database, true));
        }
        ...
        final Schema schemaToCreate = finalSchema;
        stopController.run("catalog createTable", () -> catalog.createTable(identifier, schemaToCreate, false));
    ...
    public void clearTable(String tableName) throws Exception {
        ...
        try {
            ...
            runTableDdl(tableKey, tableName, () -> {
                ...
                try {
                    stopController.run("truncate snapshot", commit::truncateTable);
                } catch (Throwable failure) {
                    ...
                    try { stopController.run("close DDL committer", commit::close);
                        ...
                        unsafeResourceOwners.add(tableKey);
                        actionFailure = appendFailure(actionFailure, closeFailure);
                    }
                }
                if (actionFailure != null) { rethrow(actionFailure); }
    ...
    private void runTableDdl(String tableKey, String tableName, TableDdlAction action)
            throws Exception {
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("DDL")) {
            throwIfStickyWriteFailure();
            List<PaimonMicroBatchCoordinator.CallbackReservation> ready = new ArrayList<>();
            Throwable ddlFailure = null;
            boolean drainSucceeded = false;
            // 释放必须有正向结果；drain 失败时 Context 仍归 Service 管理，不能交出 owner。
            boolean releasePhysicalOwner = false;
            PaimonTableWriteContext detachedContext = null;
            ...
            synchronized (lock) {
                ...
                try {
                    ...
                    drainSucceeded = true;
                    detachedContext = tableWriteContexts.remove(tableKey);
                    if (detachedContext == null) {
                        // 无本地 Context 的 DDL 仍会修改同一物理表；必须与其他 Service 的
                        // writer 共用 owner 准入。临时 truncate 也走原生 snapshot 提交路径。
                        // https://github.com/apache/paimon/blob/c05f7d1f1b1e5d37e64edab0f2978124d90b64f7/paimon-core/src/main/java/org/apache/paimon/table/sink/TableCommitImpl.java#L174
                        Table ddlTable;
                        ...
                        try {
                            detachedContext.close();
                        } finally {
                            releasePhysicalOwner = detachedContext.cleanupComplete();
                            if (!releasePhysicalOwner) {
                                unsafeResourceOwners.add(tableKey);
                                tableWriteContexts.put(tableKey, detachedContext);
                            }
                        }

                    }
                    stopController.run("DDL action", action::run);
                } catch (Exception | Error failure) {
                    ddlFailure = failure;
                    recordStickyFailure(failure);
                } finally {
                    invalidateTableDerivedCaches(tableKey, tableName);
                    if (releasePhysicalOwner && !unsafeResourceOwners.contains(tableKey)
                            && !stopController.isRetained() && !stopController.expired()) {
                        unregisterPhysicalTableOwner(tableKey);
                    }
                    if (drainSucceeded && !stopController.isRetained() && !stopController.expired()) {
    ...
    public WriteListResult<TapRecordEvent> writeRecords(List<TapRecordEvent> recordEvents,
        ...
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("writeRecords")) {
    ...
    public void afterInitialSync(TapConnectorContext connectorContext, TapTable tapTable) throws Exception {
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("afterInitialSync")) {
            String tableName = tapTable.getName();
            String database = config.getDatabase();
            String tableKey = database + "." + tableName;
            DynamicIngressGuard ingressGuard = dynamicSourceIngressGuard(tableKey, tableName);
    ...
    public void processHeartbeat(HeartbeatEvent heartbeat) throws Exception {
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("heartbeat")) {
            try {
                if (heartbeat == null) {
    ...
    private List<PaimonMicroBatchCoordinator.CallbackReservation> commitOrConfirmLocked(
            ...
            String trigger,
            boolean uninterruptibleCleanup) throws Exception {
        stopController.checkAction("business commit attempt");
        ...
        try {
            ...
            while (retry <= COMMIT_RETRY_LIMIT) {
                stopController.checkAction("commit retry");
                try {
                    while (true) {
                        try {
                            stopController.run("commit retry wait", retryWaiter::awaitRetry);
                            break;
                        } catch (InterruptedException interrupted) {
                            if (!uninterruptibleCleanup) {
                                Thread.currentThread().interrupt();
                                PaimonFailures.append(interrupted, firstFailure);
                                recordStickyFailure(interrupted);
                                throw interrupted;
                            }
                            cleanupInterruption.compareAndSet(null, interrupted);
                            if (Thread.currentThread() == stopController.worker) {
                                stopController.workerInterrupted = true;
                            }
                            Thread.interrupted();
                        }
                    }
                    if (!writeContext.hasPendingCommit()) {
                        ...
                        PaimonFailures.append(firstFailure, missingPending);
                        break;
                    }
                    writeContext.retryPendingCommit();
                    ...
                    throw interrupted;
                } catch (Error retryFailure) {
                    PaimonFailures.append(retryFailure, firstFailure);
                    recordStickyFailure(retryFailure);
                    throw retryFailure;
                } catch (Exception retryFailure) {
                    PaimonFailures.append(firstFailure, retryFailure);
                    retry++;
                }
            }
            recordStickyFailure(firstFailure);
    ...
    private List<PaimonMicroBatchCoordinator.CallbackReservation> publishSuccessfulCommit(
        ...
        stopController.checkAction("publish business commit");
        return microBatchCoordinator.publishCommit(target, completedAt, stopController);
    }
    ...
    private void executeCallbacks(
        ...
        while (!ready.isEmpty()) {
            ...
            synchronized (callbackExecutionLock) {
                stopController.checkAction("admit offset callback");
                PaimonStopController stopOperation = stopDrain ? closeOperation : null;
                PaimonServiceLifecycle.ConsumerPermit permit = lifecycle.tryStartConsumer(
                        stopDrain,
                        // 最终 Compaction 阶段不能再开始业务 offset callback。
                        () -> stopOperation == null || !stopOperation.businessDrained,
                        () -> {
                ...
                try (PaimonServiceLifecycle.ConsumerPermit ignored = permit) {
                    try {
                        ...
                        stopController.run("offset callback", () -> callback.accept(reservation.payload()));
                        stopController.checkAction("publish offset callback completion");
                        PaimonMicroBatchCoordinator.CallbackReservation next =
                                microBatchCoordinator.completeCallback(reservation, stopController);
                        if (next != null) {
                            ready.addFirst(next);
                        }
    ...
    private void flushScheduledTable(String tableKey) throws Exception {
        try {
            try (PaimonServiceLifecycle.Ingress ignored = enterIngress("scheduler")) {
    ...
    private void cleanupAfterInitializationFailure(Throwable primary) {
        try {
            ...
            if (cleanupFailure != primary) { PaimonFailures.append(primary, cleanupFailure); }
        } finally {
            if (!stopController.isRetained()) { flushOffsetCallback = null; }
        }
    }
    ...
    private PaimonTableWriteContext getOrCreateTableWriteContext(
        ...
        synchronized (lifecycleLock) {
            ...
            PaimonStopResources.Scope construction = stopResources.scope("writer " + tableKey, stopController);
            try {
                PaimonCommitStateStore.Binding binding = PaimonCommitStateStore.bind(
                        boundTaskStateMap,
                        config.getFullWarehousePath(),
                        fileStoreTable, stopController);
                if (fileStoreTable.bucketMode() == BucketMode.HASH_DYNAMIC) {
                    PaimonDynamicBucketPreflight.ensureHashDynamicValidated(
                            ...
                            stopResources.scope("preflight " + tableKey, stopController));
                }
                ...
                PaimonTableWriteContext writeContext = io.tapdata.connector.paimon.write.PaimonTableWriteContextFactory.create(
                        tableKey,
                        tableName,
                        fileStoreTable,
                        binding.commitUser(),
                        ...
                        binding.store(),
                        io.tapdata.connector.paimon.write.bucket.DefaultPaimonBucketWriterRuntimeFactory.INSTANCE,
                        writeSemanticContract, construction);
                // 已持有表级锁；短控制门禁中只安装内存引用，不进入 Factory 或 Context。
                stopController.publish("install write context", () -> {
                    tableWriteContexts.put(tableKey, writeContext);
                    paimonFieldCache.putIfAbsent(tableKey, paimonFields);
                    return null;
                });
                return writeContext;
            } catch (Exception | Error e) {
                if (construction.hasResources() || stopController.isRetained() || stopController.expired() || e instanceof io.tapdata.connector.paimon.write.PaimonTableWriteContextFactory.IncompleteCleanupException) {
                    unsafeResourceOwners.add(tableKey);
                    retainClose(stopController, e);
                    recordStickyFailure(e);
                } else {
                    construction.completed();
                    stopController.checkAction("release construction physical owner");
                    unregisterPhysicalTableOwner(tableKey);
                }
                throw e;
            }
    ...
    private DmlBatchPreflight resolveAndValidateDmlBatch(
        ...
        if (existingContext != null) {
            ...
            Table targetTable = stopController.call("catalog getTable", () -> catalog.getTable(identifier));
    ...
    private void registerPhysicalTableOwner(String tableKey, FileStoreTable table) {
        if (unsafeResourceOwners.contains(tableKey)) {
            throw new IllegalStateException("Paimon resources are not completely closed for " + tableKey);
        }
        // Paimon 1.3.2 的 HASH_DYNAMIC 不支持并发 writer，KEY_DYNAMIC 在本地维护全主键索引。
        // 此处仅做同 JVM 互斥；A -> B 或重新调度回 A 都必须满足旧实例不再提交的宿主交接条件。
        // commitUser/identifier 是重试身份，不是能拒绝旧 Engine 提交的 fencing token。
        // https://github.com/apache/paimon/blob/c05f7d1f1b1e5d37e64edab0f2978124d90b64f7/paimon-common/src/main/java/org/apache/paimon/table/BucketMode.java#L40
        ...
        stopController.publish("register physical owner", () -> {
            String existing = ACTIVE_PHYSICAL_TABLE_OWNERS.putIfAbsent(physicalHash, owner);
            if (existing != null && !existing.equals(owner)) {
                throw new IllegalStateException("Another Paimon writer context already owns the target physical table");
            }
    ...
    private void unregisterPhysicalTableOwner(String tableKey) {
        if (!physicalTableByLogicalTable.containsKey(tableKey) && !unsafeResourceOwners.contains(tableKey)) { return; }
        stopController.publish("release physical owner", () -> {
            unsafeResourceOwners.remove(tableKey);
            String physicalHash = physicalTableByLogicalTable.remove(tableKey);
            if (physicalHash != null) {
                ACTIVE_PHYSICAL_TABLE_OWNERS.remove(physicalHash, serviceWriterOwner + ':' + tableKey);
            }
            return null;
        });
    }
    ...
    private DynamicIngressGuard dynamicSourceIngressGuard(String tableKey, String tableName) throws Exception {
        ...
        if (existing != null) {
            ...
            Table table = stopController.call("catalog getTable", () -> catalog.getTable(Identifier.create(config.getDatabase(), tableName)));
    ...
    public Object timestampToStreamOffset(List<String> tables, Long timestamp, Log log) throws Exception {
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("timestampToStreamOffset")) {
            return timestampToStreamOffsetAdmitted(tables, timestamp, log);
        }
    }

    private Object timestampToStreamOffsetAdmitted(List<String> tables, Long timestamp, Log log) throws Exception {
        ...
        for (String tableName : tables) {
            try {
                ...
                Snapshot snapshot = stopController.call("lookup snapshot timestamp", () -> snapshotManager.earlierOrEqualTimeMills(timestamp));
    ...
    public void streamRead(List<String> tables, Object offsetState, int eventBatchSize,
        ...
        for (String tableName : tables) {
            ...
            Table paimonTable;
            try {
                paimonTable = stopController.call("catalog getTable", () -> catalog.getTable(identifier));
            } catch (Catalog.TableNotExistException e) {
    ...
    public void batchRead(TapTable table, Object offsetState, int eventBatchSize,
                          java.util.function.BiConsumer<List<io.tapdata.entity.event.TapEvent>, Object> eventsOffsetConsumer,
                          TapConnectorContext connectorContext) throws Exception {
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("batchRead")) {
            batchReadAdmitted(table, offsetState, eventBatchSize, eventsOffsetConsumer, connectorContext);
        }
    }

    private void batchReadAdmitted(TapTable table, Object offsetState, int eventBatchSize,
                          java.util.function.BiConsumer<List<io.tapdata.entity.event.TapEvent>, Object> eventsOffsetConsumer,
                          TapConnectorContext connectorContext) throws Exception {
        String database = config.getDatabase();
        String tableName = table.getName();
        ...
        TableScan.Plan plan = stopController.call("plan finite scan", tableScan::plan);
        List<Split> splits = plan.splits();
        ...
        for (Split split : splits) {
            // Create record reader for this split
            try (RecordReader<InternalRow> reader = createFiniteReader(() -> readBuilder.newRead().createReader(split))) {
                ...
                while (iterator != null) {
                    ...
                    while ((row = iterator.next()) != null) {
                        ...
                        if (events.size() >= eventBatchSize) {
                            acceptReadResult(eventsOffsetConsumer, events);
    ...
    public long batchCount(TapTable table, Log log) throws Exception {
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("batchCount")) {
            return batchCountAdmitted(table, log);
        }
    }

    private long batchCountAdmitted(TapTable table, Log log) throws Exception {
        String database = config.getDatabase();
        String tableName = table.getName();
        Identifier identifier = Identifier.create(database, tableName);
    ...
    public void queryByAdvanceFilter(TapTable table, io.tapdata.pdk.apis.entity.TapAdvanceFilter filter,
        ...
        // 查询 reader 与结果回调均持有准入；STOP 必须等待查询退出才能清理 Catalog。
        try (PaimonServiceLifecycle.Ingress ignored = enterIngress("queryByAdvanceFilter")) {
            queryByAdvanceFilterAdmitted(table, filter, consumer, log);
        }
    }

    private void queryByAdvanceFilterAdmitted(TapTable table,
            io.tapdata.pdk.apis.entity.TapAdvanceFilter filter,
            java.util.function.Consumer<io.tapdata.pdk.apis.entity.FilterResults> consumer,
            Log log) throws Exception {
        String database = config.getDatabase();
        String tableName = table.getName();
        ...
        // Plan all splits
        Plan plan = stopController.call("plan finite scan", tableScan::plan);
        List<Split> splits = plan.splits();
        ...
        for (Split split : splits) {
            // Create record reader for this split
            try (RecordReader<InternalRow> reader = createFiniteReader(() -> tableRead.createReader(split))) {
                // Read records from this split
                RecordReader.RecordIterator<InternalRow> iterator = reader.readBatch();
                    ...
                    while ((row = iterator.next()) != null) {
                        ...
                        if (matchesFilter(data, filter)) {
                            ...
                            if (filterResults.resultSize() >= batchSize) {
                                acceptReadResult(consumer, filterResults);
    ...
    public void close() throws Exception {
        PaimonStopController operation = startCloseOperation();
        if (Thread.currentThread() == operation.worker) {
            // stop-drain callback 重入不能等待自己，也不能发布终态或释放 owner。
            return;
        }
        boolean interrupted = false;
        while (!operation.isFinished()) {
            try {
                PaimonStopController.FinalAttempt cancellation = operation.pollCancellation();
                if (cancellation != null) {
                    safeCloseInfo(closeLogFields(operation, "cancel-requested"));
                    cancellation.requestCancellation();
                }
                if (operation.expired()) {
                    retainClose(operation, operation.timeoutFailure());
                    break;
                }
                operation.completed.await(Math.min(TimeUnit.MILLISECONDS.toNanos(50),
                        Math.max(1, operation.remainingNanos())), TimeUnit.NANOSECONDS);
                if (!operation.isFinished()) { logCloseProgress(operation, false); }
            } catch (InterruptedException interruption) {
                interrupted = true;
                recordCloseFailureAndFence(operation, interruption);
            } catch (Throwable controlFailure) {
                retainClose(operation, controlFailure);
            }
        }
        logTerminal(operation);
        Throwable failure = closeFailure(operation);
        if (interrupted) { Thread.currentThread().interrupt(); }
        if (failure != null) { rethrow(failure); }
    }

    private PaimonStopController startCloseOperation() {
        synchronized (closeCoordinationLock) {
            if (closeOperation != null) { return closeOperation; }
            PaimonStopController operation = stopController;
            if (operation.isFinished()) { closeOperation = operation; return operation; }
            operation.start();
            Thread worker = new Thread(CLOSE_WORKER_THREAD_GROUP, () -> performClose(operation),
    ...
    private void performClose(PaimonStopController operation) {
        try { performCloseWorker(operation); }
        finally {
            // Paimon ExecutorUtils.gracefulShutdown 同样恢复中断；Connector 延迟到 worker
            // 最外层退出，以便有限预算内继续取得资源终止证明，不照搬 shutdownNow 后即返回。
            // https://github.com/apache/paimon/blob/c05f7d1f1b1e5d37e64edab0f2978124d90b64f7/paimon-common/src/main/java/org/apache/paimon/utils/ExecutorUtils.java#L63-L72
            if (operation.workerInterrupted) { Thread.currentThread().interrupt(); }
        }
    }

    private void performCloseWorker(PaimonStopController operation) {
        try {
            lifecycle.beginStopping();
            operation.checkAction("begin close worker");
            setClosePhase(operation, "<service>", "DRAIN");
            ...
            retainClose(operation, shutdownFailure);
            return;
        }
        ...
        // close-vs-init/DDL exclusion while the PDK caller reports progress until cleanup completes.
        synchronized (this) {
            if (!operation.isRetained()) { performDrainAndCleanup(operation); }
        }
    }

    private void performDrainAndCleanup(PaimonStopController operation) {
        try {
            for (String tableKey : new ArrayList<>(tableWriteContexts.keySet())) {
                try {
                    setClosePhase(operation, tableKey, "DRAIN");
                    operation.checkAction("drain table");
            ...
            operation.businessDrained = true;
            try {
                cleanupGracefully(operation, closeFailure(operation) == null);
            } catch (Throwable cleanupFailure) {
                recordCloseFailure(operation, cleanupFailure);
            }
            if (operation.isRetained()) { return; }
            dynamicSourceIngressGuards.clear();
            activeConnectorContext = null;
            boundTaskStateMap = null;
            flushOffsetCallback = null;
            publishCloseCompletion(operation);
        }
    }

    private void cleanupGracefully(PaimonStopController operation, boolean businessBarrier) throws Exception {
        Throwable failure = null;
        for (Map.Entry<String, PaimonTableWriteContext> entry :
                new LinkedHashMap<>(tableWriteContexts).entrySet()) {
            String tableKey = entry.getKey();
            PaimonTableWriteContext context = entry.getValue();
            operation.checkAction("cleanup table");
            try {
                // 业务屏障之后仍必须对零业务行 Context 执行最终 prepare。该调用本身可以触发
                // Compaction，因此不能像旧协议一样预先 broadcast shutdown。
                ...
                PaimonTableWriteContext.StopOutcome result = context.closeForStop(
                        businessBarrier && closeFailure(operation) == null,
                        new PaimonTableWriteContext.CloseObserver() {
                            @Override
                            public void phase(String phase) { setClosePhase(operation, tableKey, phase); }
                            @Override
                            public void compactionDiscarded(long identifier, Throwable failure) {
                                PaimonStopLog.offer(log, closeLogFields(operation, "compaction-discarded")
                                        + " identifier=" + identifier, failure);
                            }
                            @Override
                            public void compactionTerminated() { safeCloseInfo(closeLogFields(operation,
                                        operation.finalCancellationRequested() ? "cancel-terminated" : "compaction-terminated")); }
                            @Override
                            public void compactionCancellationRequested() { safeCloseInfo(closeLogFields(operation, "cancel-requested")); }
                            @Override
                            public void spillClosed() { safeCloseInfo(closeLogFields(operation, "spill-closed") + " dirs=" + context.spillDirs()); }
                        });
                if (result == PaimonTableWriteContext.StopOutcome.SUCCESS_COMPACTION_DISCARDED) {
                    operation.discardedTables++;
                }
            } catch (Throwable contextFailure) {
                failure = appendFailure(failure, contextFailure);
            } finally {
                if (context.cleanupComplete() && !operation.isRetained()) {
                    tableWriteContexts.remove(tableKey, context);
                    unregisterPhysicalTableOwner(tableKey);
                } else {
                    unsafeResourceOwners.add(tableKey);
                }
        ...
        if (!unsafeResourceOwners.isEmpty() || !tableWriteContexts.isEmpty()
                || stopResources.size() > (catalogScope == null ? 0 : 1)) {
            retainClose(operation, failure == null ? new IllegalStateException("Resource close proof incomplete") : failure);
            return;
        }
        operation.checkAction("publish cleanup proof");
        for (String tableKey : new ArrayList<>(physicalTableByLogicalTable.keySet())) {
            if (!unsafeResourceOwners.contains(tableKey) && !tableWriteContexts.containsKey(tableKey)) {
                unregisterPhysicalTableOwner(tableKey);
            }
        }
        commitLocks.clear();
        ...
        if (catalog != null && tableWriteContexts.isEmpty() && unsafeResourceOwners.isEmpty()) {
            setClosePhase(operation, "<service>", "CLOSE_SERVICE");
            try {
                operation.run("close catalog", catalog::close);
                operation.checkAction("publish catalog close proof");
                catalog = null;
                if (catalogScope != null) { catalogScope.completed(); catalogScope = null; }
            } catch (Throwable catalogFailure) {
                failure = appendFailure(failure, catalogFailure);
                retainClose(operation, failure);
            }
        }
        if (failure != null) { rethrow(failure); }
    }

    private void setClosePhase(PaimonStopController operation, String tableKey, String phase) {
        PaimonStopController.Progress previous = operation.progress;
        if (operation.startLogged && previous.tableKey.equals(tableKey) && previous.phase.equals(phase)) { return; }
        operation.progress = new PaimonStopController.Progress(tableKey, phase, operation.now());
        logCloseProgress(operation, true);
    }

    private void logCloseProgress(PaimonStopController operation, boolean transition) {
        long now = operation.now();
        long previous = operation.lastProgressLog.get();
        if (!transition && (now - previous < TimeUnit.SECONDS.toNanos(STOP_PROGRESS_INTERVAL_SECONDS)
                || !operation.lastProgressLog.compareAndSet(previous, now))) { return; }
        if (transition) { operation.lastProgressLog.set(now); }
        String event = transition ? (operation.startLogged ? "phase-changed" : "start") : "waiting";
        operation.startLogged = true;
        safeCloseInfo(closeLogFields(operation, event));
    }

    private String closeLogFields(PaimonStopController operation, String event) {
        long now = operation.now();
        PaimonStopController.Progress progress = operation.progress;
        return "[paimon-stop] event=" + event + " table=" + progress.tableKey + " owner=" + serviceWriterOwner
                + " phase=" + progress.phase + " elapsedMs=" + TimeUnit.NANOSECONDS.toMillis(now - operation.startedNanos)
                + " phaseElapsedMs=" + TimeUnit.NANOSECONDS.toMillis(now - progress.startedNanos)
                + operation.diagnostics();
    }

    private void safeCloseInfo(String message) { PaimonStopLog.offer(log, message, null); }

    private void retainClose(PaimonStopController operation, Throwable failure) {
        operation.retain(failure, this);
        logTerminal(operation);
    }

    private void publishCloseCompletion(PaimonStopController operation) {
        if (operation.isFinished()) { return; }
        if (operation.expired() || !stopResources.isEmpty() || !unsafeResourceOwners.isEmpty()
                || !tableWriteContexts.isEmpty() || catalog != null) {
            retainClose(operation, operation.expired() ? operation.timeoutFailure()
                    : operation.failure() != null ? operation.failure()
                    : new IllegalStateException("Resource close proof incomplete"));
            return;
        }
        lifecycle.publishClosed(operation.failure());
        operation.complete(operation.failure());
        logTerminal(operation);
    }

    private void logTerminal(PaimonStopController operation) {
        PaimonStopController.Terminal terminal = operation.terminal();
        if (terminal == null || !operation.terminalLogged.compareAndSet(false, true)) { return; }
        String outcome = terminal.retained ? "FAILED_RETAINED" : terminal.failure != null ? "FAILED"
                : terminal.discardedTables > 0 ? "SUCCESS_COMPACTION_DISCARDED" : "SUCCESS";
        if (terminal.retained && terminal.timedOut) {
            PaimonStopLog.offer(log, "[paimon-stop] event=timeout-retained owner=" + serviceWriterOwner
                    + " phase=" + terminal.phase + " table=" + terminal.table + terminal.diagnostics
                    + " message=STOP 失败返回；资源/owner保留；需要确认旧进程退出", terminal.failure);
        }
        PaimonStopLog.offer(log, "[paimon-stop] event=finished outcome=" + outcome
                + " owner=" + serviceWriterOwner + " phase=" + terminal.phase + " table=" + terminal.table
                + " elapsedMs=" + TimeUnit.NANOSECONDS.toMillis(terminal.elapsedNanos)
                + " discardedTables=" + terminal.discardedTables + " inFlight=" + terminal.inFlight
                + terminal.diagnostics
                + " droppedLogs=" + PaimonStopLog.droppedCount()
                + (terminal.failure == null ? " message=正常退出" : " message=退出失败")
                + (terminal.retained ? " resources=保留至进程退出" : ""), terminal.failure);
    }

    private void awaitSchedulerTermination(PaimonStopController operation) {
        while (true) {
            operation.checkAction("await scheduler termination");
            try {
    ...
    private void awaitLifecycleQuiescence(PaimonStopController operation) {
        while (!lifecycle.isQuiescent()) {
            operation.checkAction("await lifecycle quiescence");
            try {
    ...
    private void recordCloseFailureAndFence(PaimonStopController operation, Throwable failure) {
        operation.recordFailure(failure);
        stickyWriteFailure.compareAndSet(null, failure);
    }
    private void recordCloseFailure(PaimonStopController operation, Throwable failure) {
        if (failure instanceof InterruptedException && Thread.currentThread() == operation.worker) {
            operation.workerInterrupted = true;
        }
        operation.recordFailure(failure);
    }
    private Throwable closeFailure(PaimonStopController operation) { return operation.failure(); }

    private PaimonServiceLifecycle.Ingress enterIngress(String action) throws Exception {
        if (stopController.isFinished() && stopController.failure() != null) { rethrow(stopController.failure()); }
        if (stopController.isStarted()) { throw new IllegalStateException("Paimon service is stopping: " + action); }
        stopController.checkAction(action);
        return lifecycle.enter(action);
    }

    private RecordReader<InternalRow> createFiniteReader(
            PaimonStopController.CheckedSupplier<RecordReader<InternalRow>> factory) throws Exception {
        PaimonStopResources.Scope scope = stopResources.scope("finite reader", stopController);
        try { return new PaimonGuardedReader<>(scope.create("allocate reader", factory), scope); }
        catch (Exception | Error failure) {
            if (scope.hasResources()) { scope.retain(failure); } else { scope.completed(); }
            throw failure;
        }
    }
    private <T> void acceptReadResult(java.util.function.Consumer<T> consumer, T value) throws Exception {
        stopController.run("finite reader callback", () -> consumer.accept(value));
    }
    private <T> void acceptReadResult(java.util.function.BiConsumer<T, Object> consumer, T value) throws Exception {
        stopController.run("finite reader callback", () -> consumer.accept(value, null));
    }
