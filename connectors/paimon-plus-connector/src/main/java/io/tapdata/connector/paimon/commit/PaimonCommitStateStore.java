 ...
 * takes the maximum of task state and the latest retained same-user snapshot. Source: {@code
 * paimon-flink/paimon-flink-common/src/main/java/org/apache/paimon/flink/sink/
 * CommitterOperator.java#initializeState}, lines 114-147; and {@code
 * paimon-core/src/main/java/org/apache/paimon/utils/
 * SnapshotManager.java#latestSnapshotOfUserFromFilesystem}, lines 586-630. Baseline: {@code
 ...
 */
public final class PaimonCommitStateStore implements PaimonTableWriteContext.CommitStateStore {
    ...
    public static String physicalTableHash(String physicalLocation) {
    ...
    /**
     * Returns the next Paimon commit identifier to persist after {@code snapshot}. The identifier is
     * read once, validated against {@link Long#MAX_VALUE} (exhausted) and negative values (corrupt
     * snapshot), then incremented. Used by both the bind path ({@link PaimonCommitStateStore}) and
     * the restart path ({@link PaimonTableWriteContextFactory}) so both observe the same invariants.
     */
    public static long nextIdentifier(Snapshot snapshot) {
    ...
    public static State parse(Object stored) {
    ...
    public static final class Binding {
        ...
        public long nextCommitIdentifier() {
            return nextCommitIdentifier;
        }

        public PaimonCommitStateStore store() {
            return store;
        }
