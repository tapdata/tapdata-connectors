    ...
    protected void upsert(Map<String, Object> after, WriteListResult<TapRecordEvent> listResult) throws SQLException {
        ...
        if (!containsNull) {
            ...
            try (ResultSet resultSet = queryStatement.executeQuery()) {
                if (resultSet.next()) {
                    justUpdate(DbKit.getAfterForUpdate(after, new HashMap<>(), allColumn, uniqueCondition,autoIncFields), DbKit.getBeforeForUpdate(after, new HashMap<>(), allColumn, uniqueCondition), listResult);
                } else {
                    justInsert(after);
                }
            }
