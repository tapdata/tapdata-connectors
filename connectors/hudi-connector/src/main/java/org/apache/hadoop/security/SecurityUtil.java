...
public final class SecurityUtil {
   ...
   * and {@link CommonConfigurationKeysPublic#HADOOP_SECURITY_DNS_NAMESERVER_KEY}
   *
  ...
  private static ServiceLoader<SecurityInfo> securityInfoProviders = 
    ServiceLoader.load(SecurityInfo.class, UserGroupInformation.HadoopLoginModule.class.getClassLoader());
  private static SecurityInfo[] testProviders = new SecurityInfo[0];

  public static void setSecurityInfoProviders(SecurityInfo securityInfo) {
    if (null == securityInfo) return;
    try {
      Class<ServiceLoader> utilClass = ServiceLoader.class;
      Field field = utilClass.getDeclaredField("providers");
      field.setAccessible(true);
      LinkedHashMap<String,SecurityInfo> providers = (LinkedHashMap<String,SecurityInfo>)field.get(securityInfoProviders);
      if (null == providers) {
        providers = new LinkedHashMap<>();
        field.set(securityInfoProviders, providers);
      }
      providers.put(AnnotatedSecurityInfo.class.getName(), securityInfo);
